...
static struct {
    ...
    { VE_MINOR_PT, "vencrypt_pt" },
    { VE_MINOR_CT, "vencrypt_ct" }
};


static int vencrypt_open(struct inode *inode, struct file *fp)
{
    vencrypt_device_data_t *vencrypt_data = (vencrypt_device_data_t *)fp->private_data;
    printk("VENCRYPT: %s\n", __func__);
...
static ssize_t vencrypt_read(struct file *fp, char __user *user_buffer, size_t size, loff_t *offset)
{
    vencrypt_device_data_t *vencrypt_data = (vencrypt_device_data_t *)fp->private_data;
    printk("VENCRYPT: %s\n", __func__);
...
static ssize_t vencrypt_write(struct file *fp, const char *buffer, size_t size, loff_t *offset)
{
    vencrypt_device_data_t *vencrypt_data = (vencrypt_device_data_t *)fp->private_data;
    printk("VENCRYPT: %s\n", __func__);
...
int vencrypt_init(void)
{
    ...
    printk("VENCRYPT: %s\n", __func__);
    retval = alloc_chrdev_region(&device_number, 0, VE_MAX_DEVICES, "vencrypt");
    if (retval) {
    ...
    for (ind=0; ind<(sizeof(nodes)/sizeof(nodes[0])); ind++) {
        ...
        if (retval) {
            ...
            device_create(vencrypt_class, NULL, dev, NULL, "%s", nodes[ind].name);
        }
